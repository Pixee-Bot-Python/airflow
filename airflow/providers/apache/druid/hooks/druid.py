...
class DruidHook(BaseHook):
    ...
    def submit_indexing_job(
        ...
        req_index = requests.post(url, data=json_index_spec, headers=self.header, auth=self.get_auth(), timeout=60)
        ...
        sec = 0
        while running:
            req_status = requests.get(druid_task_status_url, auth=self.get_auth(), timeout=60)

            self.log.info("Job still running for %s seconds...", sec)

            if self.max_ingestion_time and sec > self.max_ingestion_time:
                # ensure that the job gets killed if the max ingestion time is exceeded
                requests.post(f"{url}/{druid_task_id}/shutdown", auth=self.get_auth(), timeout=60)
